...
class UserDetailsController extends Controller
{
    ...
    public function userDetailsModel()
    {
    ...
    public function children(Request $request)
    {
        $page_perm = $this->admin->allPageRoles($this->module_slug);
        $id = $request->input('id');
        $field_data = $this->userDetailsModel();
        ...
        return view('vendor.profilehub.admin.users.profile.groupChildren', compact('group', 'children', 'page_perm','page_title'));
    }
    ...
    public function createrecord(Request $request, $bypass = false)
    {
        ...
        return redirect()->route('profilehub::admin.userdetails.index');
    }
    public function manage(Request $request)
    {
        ...
        if ($request->input('function') != '') {
            ...
            return redirect()->action('UserDetailsController@children', ['id' => $request->post('group_id')]);
        }
    }
    ...
    public function move(Request $request)
    {
        ...
        if ($type == 'field-parent') {
            ...
            $replace = DB::select(" SELECT * FROM `user_field` WHERE `group_sequence` = '" . $replace_seq . "' " . $field_query . " ");
        }
        //echo " SELECT * FROM `user_field` WHERE `group_sequence` = '" . $replace_seq . "' " . $field_query . " <br />";
        //echo '' . $direction . '  ' . $replace_seq . '<br />';
        //echo $replace[0]->field_id . ' replace';
        //die();
        ...
        //echo 'up ' . $table_name . ' field_id ' . $field_id . ' ' . $table_collumn . ' ' . $replace_seq . '<br />';
        $field_data->up_anything($table_name, 'field_id', $field_id, $table_collumn, $replace_seq);
        //downgrade the previous new sequence
        //echo 'down ' . $table_name . ' field_id ' . $replace[0]->field_id . ' ' . $table_collumn . ' ' . $sequence . '<br />';
