                ...
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h5 class="card-title text-uppercase fw-bold">
                            <i class="fa fa-solid fa-align-justify"></i> Additional Profile Fields {{ $group_id > 0 && isset($group) ? '- ' . $group->group_name.' Group' : '' }}
                        </h5>
                        <div>
                            @if($page_perm['create'])
                        ...
                        <tbody>
                          @foreach($user_fields as $field)
                            ...
                            @else
                              <td>
                                  @if($page_perm['update'])
                                    <a class="" href="{{route('profilehub.admin.users.profile.userdetails.manage.move',['id'=> $field->field_id, 'seq'=> $field->sequence, 'type'=> 'field-parent', 'dir'=> 'down', 'group'=> $field->group_id])}}">
