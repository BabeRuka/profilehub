...
<div class="mt-3">
 ...
 <div class="mt-3 {{ $glayout == 'cards' ? 'card' : '' }}">
     <div class="">
         
             <div class="row">
                 @foreach ($all_groups as $rowid => $group)
                     @php
                         $gs = $page_data->where('page_module', $group->group_id);
                         $group_enabled = $gs->first() ? $gs->where('page_key', 'group_enabled') : 1;
                         $genabled = $group_enabled != null && $group_enabled->first()
                         ...
                         <div class="col-xl-12 col-lg-12 col-md-6 col-sm-12 mb-3">
                             ...
                             <div class="{{ $glayout == 'cards' ? 'card-body' : 'row card-body' }}">
                                 @php
                                     ...
                                     $has_fields = $userdetails->user_field(null, $group->group_id);
                                 @endphp
                                 @if($has_fields)
                                    @foreach ($has_fields as $field)
                                        @php
                                        ...
                                        @if ($input_enabled > 0)
                                            @php
                                                echo $glayout == 'form-group' ? '<div class="' . $group_input . ' mb-3">' : '';
                                            @endphp
