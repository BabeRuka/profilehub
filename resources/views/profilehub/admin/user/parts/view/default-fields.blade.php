...
<div class="row mb-6">
    @foreach ($page_defaults as $default)
        ...
        @if ($show_input > 0)
            ...
            @else
                <div class="{{ $default == 'user_bio' ? 'col-12' : $groupcard_class }} mb-3">
                    <label
                        ...
                        <div class="form-line">
                            <div class="form-control pb-3 mb-1">
                                @php
                                    ...
                                    if($has_detail){
                                        echo stripslashes(optional($user)->$default); 
                                    }else{
                                        echo 'No ' . ucwords(str_replace('_', ' ', $default)) . ' found.'; 
                                    }
                                @endphp
            ...
            <div class="form-group">
                <div class="form-line">
                    <div class="form-control pb-3 mb-3">
                        @php
                            $has_detail = optional($user_detail)->$detail; 
                            if($has_detail){
                                if ($detail == 'profile_pic' || $detail == 'user_avatar') { 
                                    $profile_pic = isset($user_detail) && $user_detail->$detail != ''
                                    ? $url_img . '/' . $user_detail->$detail
                                ...
                                echo 'No ' . ucwords(str_replace('_', ' ', $detail)) . ' found.'; 
                            } 
                        @endphp 
                    </div>
                </div>
